...
export default function DetailPetugas() {
    ...
    async function loadData() {
        ...
        try {
            await axios({
                method: 'GET',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/petugas-detail?id=${decId}`,
                headers: {
                ...
                setNamaLengkap(res.data.users.name);
                if(res.data.users.user_details != null) {
                    if(res.data.users.user_details.photo != null) {
                        setPhotoPetugas(`${process.env.NEXT_PUBLIC_API_BACKEND}/` + res.data.users.user_details.photo.replace("public", "storage"));
                    }
                    if(res.data.users.user_details.binbinti != null) {
                        setBinBinti(res.data.users.user_details.binbinti);
                    }
                    if(res.data.users.user_details.last_name != null) {
                        setNamaBinBinti(res.data.users.user_details.last_name);
                    }
                    if(res.data.users.user_details.address != null) {
                        setAlamat(res.data.users.user_details.address);
                    }
                }
                setNoHp(res.data.users.phone.replace('+62', ''));
                
                if(res.data.users.roles.length > 0) {
                    setRole(res.data.users.roles.map(function(roles, i) {
                        return {
                            label: roles.name,
                            value: roles.id,
                        }
                    ...
                    var arrSelectRole = [];
                    res.data.users.roles.map(function(roles, i) {
                        arrSelectRole.push(roles.name);
                    });
                    if(arrSelectRole.includes('Petugas')) {
                        setHasRolePetugas(true);
                        if(res.data.users.user_has_petugas.length > 0) {
                            setPetugas(res.data.users.user_has_petugas.map(function(petugases, i) {
                                return {
                                    label: petugases.petugas_name,
                                    value: petugases.petugas_name,
                                }
                            }));
                        }
                    } else {
    ...
    const submitSimpanPetugas = async (event) => {
        ...
        if(lanjut) {
            ...
            try {
                ...
                await axios({
                    method: 'POST',
                    url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/petugas-detail-simpan`,
                    headers: {
