...
export default function DetailArtikel() {
    ...
    async function loadData() {
        ...
        try {
            await axios({
                method: 'GET',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/article/detail?id=${decId}`,
                headers: {
                ...
                if(res.data.article.header_picture != null) {
                    setPhotoArtikel(`${process.env.NEXT_PUBLIC_API_BACKEND}/` + res.data.article.header_picture.replace("public", "storage"));
                }
                if(res.data.article.title != null) {
                    setTitle(res.data.article.title);
                }
                if(res.data.article.description != null) {
                    setDescription(res.data.article.description);
                }
                if(res.data.article.expired_days != null) {
                    var date_1 = new Date(formatTgl(res.data.article.created_at));
                    date_1.setDate(date_1.getDate() + res.data.article.expired_days);
                    ...
                    setExpiredDays(res.data.article.expired_days);
                } else {
                    setHariBerakhir(expiredDays);
                }
                if(res.data.article.active != null) {
                    setActive(res.data.article.active);
                }
                setLoading(false);
            });
        } catch (err) {
        ...
        if(description == null || description == "") {
         ...
         if(lanjut) {
            ...
            try {
                ...
                await axios({
                    method: 'POST',
                    url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/article/detail/simpan`,
                    headers: {
                                                    ...
                                                    <Tooltip hasArrow bg='red.600' color={'white'} fontSize={{ sm: 'xs', md: 'sm', lg: 'sm' }} label='Upload foto!' placement='bottom-start' isOpen={openErrorPhotoArtikel}>
                                                        <Flex flexDirection={'row'} w={'100%'} h={'100%'} borderBottomRadius={{ sm: '5px', lg: '5px' }} justify="center" align="center">
                                                            ...
                                                            >
                                                                ...
                                                                {
                                                                    photoArtikel != null ?
                                                                        <Image borderRadius={{ sm: '7.5px', lg: '7.5px' }} p={'2.5px'} w={'100%'} h={'100%'} src={photoArtikel}  fallbackSrc='/broken-image.png'/>
                                                                    :
