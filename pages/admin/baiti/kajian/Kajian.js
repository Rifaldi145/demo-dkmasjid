...
export default function Kajian() {
    ...
    async function loadData(currentPage) {
        ...
        try {
            ...
            await axios({
                method: 'POST',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/kajian?page=${currentPage}`,
                headers: {
                ...
                console.log("Kajian",res);
                setCurrentPage(res.data.kajian.current_page);
                setTotalPage(res.data.kajian.last_page);
                setDataKajian(res.data.kajian);
                setLoading(false);
            });
        } catch (err) {
            setLoading(false);
    ...
    const hapusKajian = async (event) => {
        ...
        try {
            ...
            await axios({
                method: 'POST',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/kajian-hapus`,
                headers: {
                    "Content-Type": `application/json`,
                    "Accept": `application/json`,
                    "Authorization": `Bearer ${token}`
    ...
    return (
        <Flex flexDirection="column" pt={{ base: "90px", md: "85px" }} w={'100%'}>
                    ...
                    <Card overflowX={{ sm: "scroll", xl: "hidden" }} p={{ sm: 2.5, lg: 5 }}>
                        ...
                        <Flex flexDirection="column" w={'100%'}>
                            <Grid templateColumns={{ sm: 'repeat(7, 1fr)', lg: 'repeat(2, 1fr)'}}>
                                    ...
                                    <Flex flexDirection="row" height={'100%'} w="100%">
                                        ...
                                        >
                                            ...
                                            <InputRightAddon
                                                ...
                                                children={
                                                <Center>
                                                    <Text color={textColorWhite} fontSize={{ sm: 'xs', lg: 'xl' }}>
                                                        <MdSearch/>
                                                    </Text>
                                                </Center>   
                                            }/>
                                ...
                                <GridItem colSpan={{ sm: 2, lg: 1 }}>
                                    <Flex flexDirection="row" justify={'end'} align={'center'} height={'100%'} w="100%">
                                        <Link align="right" to={`/admin/baiti/tambah-kajian`}>
                                            <Button
                                                type="button"
                                                size={{ sm: 'sm', lg: 'sm' }}
                            ...
                            <Grid templateColumns={{ sm: '1fr', lg: 'repeat(2, 1fr)'}} >
                                {dataKajian.data.map(function(kajian, i) {
                                    var encId = encodeURIComponent(CryptoAES.encrypt(kajian.id.toString(), `${process.env.NEXT_PUBLIC_ENC_WORD}`).toString());

                                    return (
                                        <GridItem key={i} colSpan={1} p={1}>
                                            <Box boxShadow='base' borderRadius={{ sm: '5px', lg: '5px' }} border="1px solid #dfdfdf" p={{ sm: 2, lg: 4 }}>
                                                <Flex flexDirection="column" w={'100%'}>
                                                    <Grid templateColumns={{ sm: 'repeat(21, 1fr)', lg: 'repeat(21, 1fr)'}} pb={2}>
                                                        <GridItem colSpan={20}>
                                                            ...
                                                            <Flex flexDirection="row" w={'100%'} justify="start" align="start">
                                                                 {
                                                                    kajian.description == null ?
                                                                    <></>
                                                                    :
                                                                    <Text textStyle='isi_12'>
                                                                        {kajian.description}
                                                                    </Text>
                                                                }
                                                            ...
                                                            <Flex flexDirection="row" w={'100%'} justify="center" align="end">
                                                                <Menu p={0} m={0} size={'xs'}>
                                                                    ...
                                                                    <MenuList minWidth={'auto'} maxWidth={'auto'} size={'xs'}>
                                                                        <Link to={`/admin/baiti/detail-kajian/${encId}`}>
...

                                                    {
                                                        ...
                                                        :
                                                            ...
                                                            <Grid  templateColumns={{ sm: 'repeat(2, 1fr)', lg: 'repeat(2, 1fr)'}} pb={2}>
                                                                <GridItem colSpan={1}>
                                                     ...
                                                     <Grid pt={1} templateColumns={{ sm: 'repeat(1, 1fr)', lg: 'repeat(1, 1fr)'}}>
                                                       
                                                        <GridItem colSpan={1}>
                                                              <Flex pb={1} flexDirection={'row'} w={'100%'} justify="end" align="center">
                                                                   
                                                              </Flex>

                                                            <Flex flexDirection={'row'} w={'100%'} justify="end" align="center">
                                                                <Badge border="1px solid" bg="transparent" size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                    { kajian.active == 1 ? 'Aktif' : 'Tidak Aktif' }
                                                                </Badge>
                                                            </Flex>
                                                        </GridItem>
                                                    </Grid>
                                                ...
                                                {
                                                    kajian.start_date == null && kajian.end_date == null ?                           
                                                            <Grid pt={1} templateColumns={{ sm: 'repeat(1, 1fr)', lg: 'repeat(1, 1fr)'}} mt={{sm:'0',lg:'-6'}}>
                                                                    <GridItem colSpan={1}>
                                                                        <Flex flexDirection={'row'} w={'100%'} justify="start" align="center" mb="1">
                                                                            <Badge border="1px solid #b1b1b1" bg="transparent" size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                                {kajian.kajian_type  == 1 ? 'Kajian Rutin' : 'Tidak Aktif'}
                                                                            </Badge>
                                                                    ...
                                                                    <GridItem colSpan={1}>
                                                                        <Flex flexDirection={'row'} w={'100%'} justify="start" align="center" mb="1">
                                                                            <Badge border="1px solid #b1b1b1" bg="transparent" size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                               Hari : {kajian.hari} &amp; Jam : {kajian.reminder} - {kajian.waktu} 
                                                                            </Badge>
                                                                        </Flex>
                                                    ...
                                                    <Grid pt={1} templateColumns={{ sm: 'repeat(1, 1fr)', lg: 'repeat(1, 1fr)'}} mt={{sm:'0',lg:'-6'}}>
                                                        <GridItem colSpan={1}>
                                                            <Flex flexDirection={'row'} w={'100%'} justify="start" align="center" mb="1">
                                                                <Badge variant='solid' bg={'#8f82bc'} size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                    Mulai {tglIndo(kajian.start_date)}
                                                                </Badge>
                                                                &nbsp;-&nbsp;          
                                                                <Badge variant='solid' colorScheme='yellow' size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                    Selesai {tglIndo(kajian.end_date)}
                                                                </Badge>
                                                            </Flex>
                                                            <Flex flexDirection={'row'} w={'100%'} justify="start" align="center">
                                                            ...
                                                            <GridItem colSpan={1}>
                                                                        <Flex flexDirection={'row'} w={'100%'} justify="start" align="center" mb="1">
                                                                            <Badge border="1px solid #b1b1b1" bg="transparent" size={'xs'} fontSize={{ sm: '2xs', lg: 'xs' }}>
                                                                                Jam : {kajian.reminder} - {kajian.waktu}
                                                                            </Badge>
            ...
            <Modal closeOnOverlayClick={false} isCentered size={'xs'} isOpen={openDialogHapus}>
                ...
                />
                <chakra.form onSubmit={hapusKajian}>
