...
import MenujuAdzan from "./MenujuAdzan";
import toast from "../../../../components/Toast";

export default function SholatJumat() {
    ...
    async function loadData() {
        ...
        try {
            await axios({
                method: 'GET',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/edit-display/sholat-jumat`,
                headers: {
    ...
    const submitTampilkanTv = async (event) => {
        ...
        try {
            await axios({
                method: 'POST',
                url: `${process.env.NEXT_PUBLIC_API_BACKEND}/api/baitiV1/edit-display/sholat-jumat/push-tv`,
                headers: {
                    "Content-Type": `application/json`,
                    "Accept": `application/json`,
                    "Authorization": `Bearer ${token}`
                    ...
                    <Card overflowX={{ sm: "scroll", xl: "hidden" }} p={{ sm: 2.5, lg: 5 }}>
                        <Tabs variant='enclosed' size={'md'} defaultIndex={ callbackTab == null ? 0 : callbackTab }>
                            ...
                            <TabList overflowX="scroll">
                                <Tab _selected={{ bg: "#B3A5DA" }} color={textColor} borderRadius={'5px'} fontWeight={'bold'} fontSize={{ sm: 'xs', lg: 'sm' }}>Khotbah</Tab>
                                <Tab  _selected={{ bg: "#B3A5DA" }} color={textColor} borderRadius={'5px'} fontWeight={'bold'} fontSize={{ sm: 'xs', lg: 'sm' }}>Menuju Adzan</Tab>
